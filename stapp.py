...
event_api_key = st.secrets["API_KEY"]  # Ensure the API key is correctly referenced
...
def main():
    ...
    date_start = st.date_input('Start Date', key='start_date_input')
    st.markdown('*Enter End Date (YYYY-MM-DD):*')
    date_end = st.date_input('End Date', key='end_date_input')
